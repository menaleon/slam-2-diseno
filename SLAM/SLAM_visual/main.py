...
class OpticalFlowSLAM:
    ...
    def save_trajectory_outputs(self, trajectory, input_video_path):
        ...
        output_dir = os.path.join("results", tipo_SLAM, timestamp)
        ...
        info_text = (
            ...
            f"Prom. matches per pose: {avg_tracked_points:.1f}\n"
            #f"Éxito triangulación: {triangulation_success_rate:.2%}\n"
            f"Mean movement between keyframes: {avg_translation:.2f} m"
        )
        plt.gcf().text(0.02, 0.72, info_text, fontsize=10, bbox=dict(facecolor='white', alpha=0.8))

        plt.xlabel("X Position")
        plt.ylabel("Z Position")
        plt.title(f"{self.name} for {Path(input_video_path).name}")
        plt.legend()
        plt.axis("equal")
        plt.grid(True)
        plt.tight_layout()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Use: python3 main.py <ruta_al_video>")
        sys.exit(1)
    ...
    print("Global optimization completed and saved trajectory.")
